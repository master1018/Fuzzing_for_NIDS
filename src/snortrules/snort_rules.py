...
class RuleFile(object):
    ...
    def __init__(self, rule_file):
        ...
        self.fo = open(self.rule_file, "r")

    def get_file_name(self):
        names = self.rule_file.split("/")
        return names[-1]
...
class SnortRule(object):
    ...
    def __init__(self, rule):
        ...
        self.set_rule_header()
        self.set_rule_options()
        self.parse_rule_options()

    # ------------------------------------------------------
    # Parses Snort rules and provides interfaces for use
    def set_rule_header(self):
...
class SnortRuleAttr(object):
        ...
        if 'content' in self.snortrule.opt_keyword_list:
            ...
            for item in content_list:
                content = {'match': item.split(',')[0].strip('"'), 'nocase': 0, 'rawbytes': 0, 'fast_pattern': 0,
                           'depth': 0,
                           'offset': 0, 'distance': 0, 'within': 0, 'sticky': '', 'neg': 0}
                if content['match'][0] == '!':
                    content['neg'] = 1
    ...
    def get_opt_isdataat(self):
        if 'isdataat' in self.snortrule.rule_opt_pairs:
            isdataat = {'neg': 0, 'pos': 0, 'relative': 0, 'rawbytes': 0}
    ...
    def get_opt_pcre(self):
        if 'pcre' in self.snortrule.rule_opt_pairs:
            exp = self.snortrule.rule_opt_pairs["pcre"][0].strip('"')
            re = {'expression': '', 'options': '', 'sticky': ''}
            if exp[-1] == "/":
                re['expression'] = exp[1:-1]
            else:
                tmp = exp[1:]
                i = len(tmp)
                while tmp[i-1] != "/":
                    i -= 1
                re['expression'] = tmp[:i-1]
                re['options'] = tmp[i:]
            index = self.__option_index("pcre")[0]
            for key, value in self.sticky_buffer.items():
                if index < value:
                    continue
                elif index > value:
    ...
    def get_opt_byte_test(self):
        # ==TO-FIX== 'value' can be a variable name extracted from using byte_extract
        if 'byte_test' in self.snortrule.opt_keyword_list:
            tmp = self.snortrule.rule_opt_pairs['byte_test'][0].split(',')
            byte_test = {'bytes': int(tmp[0]), 'operator': tmp[1], 'value': 0, 'offset': self.__is_name_or_num(tmp[3])}
            if len(tmp) > 4:
                for elem in tmp[4:]:
                    s = elem.split()[0]
                    if s == 'relative':
                        ...
                        byte_test['value'] = hex(tmp[2])
                    elif s == 'oct':
                        byte_test['string'] = 'oct'
                        byte_test['value'] = oct(tmp[2])
                    elif s == 'dec':
                        ...
                        byte_test['bitmask'] = elem.split()[1]
                    else:
                        continue
                return byte_test
            else:
    ...
    def get_opt_byte_jump(self):
        if 'byte_jump' in self.snortrule.opt_keyword_list:
            tmp = self.snortrule.rule_opt_pairs['byte_jump'][0].split(',')
    ...
    def get_opt_byte_extract(self):
        if 'byte_extract' in self.snortrule.opt_keyword_list:
            tmp = self.snortrule.rule_opt_pairs['byte_extract'][0].split(',')
    ...
    def get_opt_byte_math(self):
        if 'byte_math' in self.snortrule.opt_keyword_list:
            tmp = self.snortrule.rule_opt_pairs['byte_math'][0].split(',')
            byte_math = {}
            for elem in tmp:
                s = elem.split()
                if len(s) > 1:
